...
export class ParkingComponent implements OnInit {
  ...
  uploadUrl: string = '/api/parking/import';
  headers: object = null;
  importVisible = false;
  importList = [];
  ...
  getlist() {
    this.isLoading = true;
    this.parkingService.getlist(this.selectData, this.PageSize, this.PageIndex).subscribe(result => {
  ...
  edit(index: number) {
    ...
    var that = this;
    const modal = this.modalService.create({
      nzTitle: '编辑业主“' + this.datalist[index]['OwnerName'] + this.datalist[index]['CarNumber'] + '”的缴费单据“',
  ...
  delete(id: number) {
    this.modalService.confirm({
      ...
      nzClosable: false,
      nzOnOk: () => {
        this.parkingService.delete(id).subscribe(result => {
  ...
  download(filename: string){
    this.http.get(this.uri + '/api/parking/importfile?filename=' + filename, {responseType: 'blob', observe: 'response'}).subscribe(res => {
  ...
  export() {
    this.isLoading = true;
    this.http.post(`${this.uri}/api/parking/export`, this.selectData, { responseType: 'blob', observe: 'response' }).subscribe(res => {
      this.isLoading = false;
      let blob = new Blob([res.body], { type: "application/octet-stream" });
      let objectUrl = URL.createObjectURL(blob);
      let a = document.createElement('a');
  ...
  constructor(private modalService: NzModalService, private http: HttpClient, private msg: NzMessageService, @Inject(API_CONFIG) private uri: string, private estateService: EstateService, private parkingService: ParkingService, private oprationService: OprationService) { }
